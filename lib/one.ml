...
let exec (log : string) : int =
  ...
  let sorted_calorie_sums = List.rev (List.sort Int.compare calorie_sums) in
  let top_three_calorie_sum = 
    (List.nth sorted_calorie_sums 0) +
    (List.nth sorted_calorie_sums 1) +
    (List.nth sorted_calorie_sums 2) in
